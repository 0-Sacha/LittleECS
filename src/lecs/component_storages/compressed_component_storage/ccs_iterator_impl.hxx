    ...
    namespace CustomIterator {
        ...
        public:
            CCSIteratorNoRefNoMap(const CompressedComponentStorage* compressedComponentStorage, std::size_t currentIndexOfPage = 0, std::size_t currentPageIndexOfCurrent = 0)
                : compressed_component_storage_(compressedComponentStorage), current_index_of_page_(currentIndexOfPage), current_page_index_(currentPageIndexOfCurrent) {
        ...
        private:
            bool current_entity_is_valid() const {
                if (compressed_component_storage_ == nullptr) return false;

                if (current_index_of_page_ >= compressed_component_storage_->get_page_container().size()) return false;

                if (current_page_index_ >= CompressedComponentStorage::PAGE_SIZE) return false;
        ...
        public:
            ...
            CCSIteratorNoRefNoMap operator++(int) {
                CCSIteratorNoRefNoMap res(compressed_component_storage_, current_index_of_page_, current_page_index_);
                ++(*this);
                return res;
            }
...
#ifdef LECS_DEBUG
                bool complex_result = compressed_component_storage_ != nullptr && compressed_component_storage_ == rhs.compressed_component_storage_ &&
                                      current_index_of_page_ == rhs.current_index_of_page_ && current_page_index_ == rhs.current_page_index_;
        ...
        public:
            CCSIteratorNoRefMap(SubIterator map_ref_iterator) : map_ref_iterator_(map_ref_iterator) {}
