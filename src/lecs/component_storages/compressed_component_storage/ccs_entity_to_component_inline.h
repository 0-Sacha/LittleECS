...
namespace lecs::detail {
    ...
    public:
        CCS_EntityToComponent_Inline() : bucket_container_(), alive_entities_container_() {
            if constexpr (HAS_ENTITIES_REF) alive_entities_container_.reserve(PAGE_SIZE);
        }
    ...
    public:
        inline bool has_entity(EntityId entity) const {
            ...
            if (bucketinfo.index_of_page >= bucket_container_.size()) return false;

            const BucketRef& bucketref = bucket_container_[bucketinfo.index_of_page];

            if (bucketref == nullptr) return false;
